...
def get_variation_fn(
    ...
    def perform_variation(structures, constants, fitness, trial_structures, trial_constants, trial_fitness, X, y, rng):
        ...
        # specify the methods for local search for both the constants and structures
        structure_search = 'central'  # 'none' or 'forward' or 'central' or 'backward'
        constants_search = 'central'  # 'none' or 'forward' or 'central' or 'backward'

        # TODO: add structure and constant search to config, also make 2 different epsilons and learning rates

        structs_grad = None if structure_search == 'none' else finite_differencing(X, structure_search, temp_trial_fit,
                                                                                   temp_trial_structs,
                                                                                   temp_trial_consts, y,
                                                                                   edit_structs=True)

        consts_grad = None if constants_search == 'none' else finite_differencing(X, constants_search, temp_trial_fit,
                                                                                  temp_trial_consts,
                                                                                  temp_trial_structs, y,
                                                                                  edit_structs=False)
    ...
    def update_parameters(param, grad, temp_param, lr):
        for i in range(param.shape[0]):
            for j in range(param.shape[1]):
                param[i, j] = temp_param[i, j] + (grad[i, j] * lr) if grad is not None else temp_param[i, j]

    def finite_differencing(X, finite_difference_method, fitness, to_edit, to_leave, y, edit_structs=False):
